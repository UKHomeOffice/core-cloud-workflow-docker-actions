...
## Overview
This repository forms part of the UK Home Office Core Cloud Platform’s shared GitHub Actions tooling. It contains a reusable trivy scant composite GitHub Action used across the platform to maintain docker code.

Because of its purpose, the repository includes **intentionally invalid docker code** for testing, validation, and demonstration.

This design requires specific security guidance, explained below.

## Why This Repository Contains Invalid docker
...
* violate different docker code

These files are required to:

* validate the trivy reusable actions
...
Any production docker code must not be added to these directories, because:

* It will be invisible to SAST

* It may be mistaken for intentionally broken docker
...
This repository does not use third-party Python, Go, or docker dependencies — only GitHub Actions and shell scripts.
...
| Area                    | Policy                                               |
| ----------------------- | ---------------------------------------------------- |
| Test docker files       | Intentionally invalid & excluded from SAST           |
